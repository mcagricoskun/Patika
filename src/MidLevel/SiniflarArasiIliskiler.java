...
Bağımlılık (Dependency) / “uses a” İlişkisi / a sınıfı b sınıfını kullanır gibi // a sınıf b sınıfının metodlarını veya değişkenlerini kullanır gibi

Java'da Bağımlılık bir diğer adı ile "uses a" ilişkisi, eğer bir sınıfın metotlarından en az biri başka bir sınıfa ait
bir parametre alıyorsa, parametre alan metoda ait sınıf diğeri sınıfa bağımlıdır ya da onu kullanır (uses) denir.
...
Bu durumda programın yönetilebilirliği veya bakımı zorlaşacaktır. Bunun sebebi ise, bir sınıf içinde yapılacak herhangi
bir değişiklik bağımlı olduğu sınıfları da etkileyecektir ve bakım yükü artacaktır.
